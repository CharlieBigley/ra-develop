...
/**
 * Methods supporting a list of Components records.
 *
 * @since  0.4.0
 */
class ExtentionsModelsModel extends ListModel
{
	/**
	* Constructor.
	*
		...
		foreach ($items as $oneItem)
		{
			...
			if (isset($oneItem->subsystem_id))
			{
				...
				foreach ($values as $value)
				{
					...
					{
						$db = $this->getDbo();
						$query = "SELECT id,name FROM #__ra_sub_systems WHERE #__ra_sub_systems.id LIKE '" . $value . "' ORDER BY name";
						$db->setQuery($query);
