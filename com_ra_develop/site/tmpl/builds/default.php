...
$canCreate  = $user->authorise('core.create', 'com_ra_develop') && file_exists(JPATH_COMPONENT . DIRECTORY_SEPARATOR . 'forms' . DIRECTORY_SEPARATOR . 'extension_typeform.xml');
$canEdit    = $user->authorise('core.edit', 'com_ra_develop') && file_exists(JPATH_COMPONENT .  DIRECTORY_SEPARATOR . 'forms' . DIRECTORY_SEPARATOR . 'extension_typeform.xml');
$canCheckin = $user->authorise('core.manage', 'com_ra_develop');
$canChange  = $user->authorise('core.edit.state', 'com_ra_develop');
$canDelete  = $user->authorise('core.delete', 'com_ra_develop');
	...
	if($canDelete) {
		$wa->addInlineScript("
			...
			function deleteItem() {
				...
				if (!confirm(\"" . Text::_('COM_RA_DEVELOP_DELETE_MESSAGE') . "\")) {
					return false;
				}
			}
		", [], [], ["jquery"]);
